	...
	*/
	...
	// </EXAMPLE> ------------------------->8-----------------------
		...
		if( webSocketAddress != undefined ) {
			console.log('Ïîïûòêà óñòàíîâèòü ñîåäèíåíèå ñ ' + webSocketAddress );
			webSocket = new WebSocket( webSocketAddress );
			webSocket.onopen = function() {
				console.log("Ñîåäèíåíèå óñòàíîâëåíî.");
			};
			webSocket.onclose = function(event) {
			if (event.wasClean) {
				console.log('Ñîåäèíåíèå çàêðûòî ÷èñòî');
			} else {
				console.log('Îáðûâ ñîåäèíåíèÿ'); // íàïðèìåð, "óáèò" ïðîöåññ ñåðâåðà
			}
				console.log('Êîä: ' + event.code + ' ïðè÷èíà: ' + event.reason);
			};
			webSocket.onmessage = function(event) {
				console.log("Ïîëó÷åíû äàííûå " + event.data);
				command( event.data );
			};
			webSocket.onerror = function(error) {
				console.log("Îøèáêà webSocket: " + error.message);
			};
			// control events
